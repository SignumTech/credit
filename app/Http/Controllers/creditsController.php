...
class creditsController extends Controller
{
    public function creditWorthiness(Request $request){
        ...
        $sum = 0;
        $cutoff = 2;
        $sum += $this->businessExperienceScore(Carbon::parse($request->created_at));var_dump($sum);
        $sum += $this->preSaleEstimationScore($request->presale_estimation);var_dump($sum);
        $sum += $this->businessActivityScore(Carbon::parse($request->last_order_date));var_dump($sum);
        $sum += $this->businessTypeScore($request->business_type);var_dump($sum);
